...
if (!empty($_POST['email']) and !empty($_POST['pass']) and !empty($_POST['pass2']) and !empty($_POST['name'])) {
    if (preg_match_all('/\b[\w.%+-]+@[\w.-]+\.[a-zA-Z]{2,6}\b/', $_POST['email'])===1) {
        preg_match('/\b[\w.%+-]+@[\w.-]+\.[a-zA-Z]{2,6}\b/', $_POST['email'], $matches);
        $email=$matches[0];
        if (preg_match_all('/^[a-zA-Z][a-zA-Z0-9_]{4,15}$/', $_POST['name'])===1) {
            preg_match('/^[a-zA-Z][a-zA-Z0-9_]{4,15}$/', $_POST['name'], $matches);
            $name=$matches[0];
            if (preg_match_all('/^[a-zA-Z]\w{5,17}$/', $_POST['pass'])===1) {
                preg_match('/^[a-zA-Z]\w{5,17}$/', $_POST['pass'], $matches);
                $password=$matches[0];
                if ($password==$_POST['pass']) {
                    if ($_POST['pass']==$_POST['pass2']) {
                        ...
                        if ($con->query($sql)->num_rows > 0) {
                            ...
                            $sql="INSERT INTO user (id, name, pass, email)
VALUES (null, '".$name."', '".$password."', '".$email."')";
                            $con->query($sql);
                            $url = $_SERVER['REMOTE_HOST']."/user/login/index.php";
    ...
    <body>
        ...
        <div class="use-flexbox" style="weight:100%;height:100%;text-align: center;">
            <div style="text-align: center">
                <h1 class="font-mc" style="text-align:center">Regin</h1>
                <!--menu-->
                <div class="mdui-card" style="background-color: rgba(255,255,255,0.5);">
                    <div class="mdui-card-content">
                        <form action="/user/regin/index.php" method="post" style="text-align: left">
                            <!--Name-->
                            <div class="mdui-textfield mdui-textfield-floating-label">
                                <label class="mdui-textfield-label">用户名</label>
                                <input class="mdui-textfield-input" type="text" name="name" required/>
                                <div class="mdui-textfield-error">用户名必填</div>
                            </div>
                            <!--Email-->
                            <div class="mdui-textfield mdui-textfield-floating-label">
                            ...
                            <div class="mdui-textfield mdui-textfield-floating-label">
                                ...
                                <input class="mdui-textfield-input" type="password" name="pass2" required/>
                            </div>
                            <!--login btn-->
                            <div style="text-align:center;">
                                <input type="submit" class="mdui-btn mdui-btn-raised mdui-ripple" style="background-color: rgba(255,255,255,0.7)" value="注册"/>
                                ...
                                if (!empty($stu)) {
                                    switch ($stu) {
                                        case 'email':
                                            ...
                                            break;
                                        case 'name':
                                            echo('用户名只允许字母开头，5-16字节，字母数字下划线');
                                            // ^[a-zA-Z][a-zA-Z0-9_]{4,15}$
                                            break;
                                        case 'password':
                                            echo('密码应以字母开头，长度在6~18之间，只能包含字母、数字和下划线');
                                            // ^[a-zA-Z]\w{5,17}$
                                            break;
                                        case 'pass2':
                                            echo '两次输入的密码不一致';
